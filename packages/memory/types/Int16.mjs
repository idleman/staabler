/**
 * 16-bit signed integer type implementation with optimized byte-level access.
 * 
 * This class provides static methods for reading and writing 16-bit signed integers
...
export default class Int16 {
  ...
  /**
   * Reads a 16-bit signed integer from a buffer at the specified byte offset.
   * 
  ...
  static getValue(buffer, byteOffset = 0) {
    return (buffer[byteOffset] | (buffer[byteOffset + 1] << 8)) << 16 >> 16;
  }

  /**
   * Writes a 16-bit signed integer to a buffer at the specified byte offset.
   * 
   * @param {ArrayBuffer|TypedArray} buffer - The buffer to write to
   * @param {number} [byteOffset=0] - The byte offset to write to
   * @param {number} value - The 16-bit signed integer value to write
   */
